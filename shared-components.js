...
const dieHantarUI = {
    ...
    renderHistory(containerId, orders) {
        ...
        }).join('') || `<div class="text-center py-20 text-[10px] font-black text-gray-300 uppercase italic">Belum Ada Aktivitas</div>`;
    },
    ...
    renderLiveMap(containerId, progress, serviceName) {
        ...
        if (serviceName.includes('MOBIL') || serviceName.includes('TAXI')) icon = 'fa-car';
        if (serviceName.includes('MAKAN')) icon = 'fa-utensils';
        if (serviceName.includes('KIRIM') || serviceName.includes('BOX')) icon = 'fa-box';
...
const dieHantarRefresh = {
    init(containerId, callback, themeColor = "#ea580c") {
        ...
        indicator.className = 'flex justify-center items-center overflow-hidden transition-all duration-300 w-full bg-transparent';
        ...
        container.addEventListener('touchmove', (e) => {
            ...
            if (diff > 0 && diff < 100) { indicator.style.height = diff + 'px'; const icon = document.getElementById('ptr-icon'); if(icon) icon.style.transform = `rotate(${diff * 4}deg)`; }
        }, { passive: true });
        container.addEventListener('touchend', async () => {
            if (!pulling) return; pulling = false; const currentHeight = parseInt(indicator.style.height);
            if (currentHeight > 60) {
                indicator.style.height = '60px'; const icon = document.getElementById('ptr-icon');
                if(icon) icon.classList.add('fa-spin');
                if (callback) await callback();
                setTimeout(() => { indicator.style.height = '0px'; if(icon) icon.classList.remove('fa-spin'); }, 1000);
...
const DriverProfileComponent = { render: (containerId, data) => dieHantarUI.renderDriverCard(containerId, data) };