...
const dieHantarUI = {
          ...
          (o, index) => `
            <div onclick="openReceiptFromHistory(${orders.length - 1 - index})" class="bg-white p-5 rounded-[2.5rem] border border-gray-100 flex justify-between items-center mb-3 shadow-sm active:scale-95 transition-all cursor-pointer group">
                <div class="flex items-center gap-4">
                    ...
                    <div><p class="text-[7px] font-black uppercase ${o.status === "COMPLETED" ? "text-green-500" : "text-orange-500"} mb-1">${o.status}</p><h4 class="font-black text-xs text-zinc-800 leading-tight">${o.item}</h4><p class="text-[6px] text-gray-400 font-black uppercase mt-1">${o.date || "Tgl N/A"}</p></div>
                </div>
                <div class="text-right"><p class="font-black text-xs italic text-zinc-900">Rp ${parseInt(o.price).toLocaleString("id-ID")}</p><i class="fas fa-chevron-right text-[8px] text-gray-200 group-hover:text-orange-500 mt-1"></i></div>
            </div>`,
        )
        .join("") ||
  ...
  renderReceiptModal(order) {
    ...
    let extraInfo = order.employeeInfo
      ? `<div class="mt-4 p-4 bg-gray-50 rounded-2xl border border-gray-100"><p class="text-[7px] font-black text-gray-400 uppercase mb-2">Data Potong Gajian</p><p class="text-[9px] font-bold text-zinc-700 leading-tight">👤 ${order.employeeInfo.name} <br> 🆔 ${order.employeeInfo.nik} <br> 🏢 ${order.employeeInfo.pt}</p></div>`
      : "";
    const receiptHtml = `<div id="modal-receipt" class="fixed inset-0 bg-black/95 z-[500] flex items-center justify-center p-6 backdrop-blur-md fade-in">
                <div class="bg-white w-full max-w-sm rounded-[3.5rem] overflow-hidden shadow-2xl animate-slide-up">
                    <div class="bg-zinc-900 p-10 text-center text-white"><div class="w-16 h-16 bg-orange-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg"><i class="fas fa-receipt text-2xl text-white"></i></div><h3 class="text-xl font-black italic uppercase tracking-tighter">Struk dieHantar</h3><p class="text-[8px] font-bold text-orange-500 uppercase tracking-[0.4em] mt-2">Digital Receipt Sultan</p></div>
...
const AppRenderer = {
  ...
  renderBookingOptions() {
    ...
    if (!container) return;
    container.innerHTML = `<p class="text-[8px] font-black text-zinc-400 uppercase tracking-widest mb-4">Pilih Kelas Sultan</p>
            <button onclick="setSultanClass('Standard', 15000)" id="class-standard" class="class-btn w-full p-6 bg-orange-50 rounded-[2rem] border-2 border-orange-500 flex justify-between items-center transition-all active:scale-95">
                <div class="flex items-center gap-4"><div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-orange-600 shadow-sm"><i class="fas fa-motorcycle text-lg"></i></div><div class="text-left font-black"><h4 class="text-xs text-zinc-800 uppercase leading-none">Standard</h4><p class="text-[8px] text-gray-400 font-bold uppercase mt-1">Cepat & Irit</p></div></div><p class="text-sm font-black text-zinc-900 italic">Rp 15k</p></button>
            <button onclick="setSultanClass('Sultan Class', 25000)" id="class-sultan" class="class-btn w-full p-6 bg-gray-50 rounded-[2rem] border-2 border-transparent flex justify-between items-center transition-all active:scale-95">
                <div class="flex items-center gap-4"><div class="w-12 h-12 bg-zinc-900 rounded-2xl flex items-center justify-center text-yellow-500 shadow-sm"><i class="fas fa-crown text-lg"></i></div><div class="text-left font-black"><h4 class="text-xs text-zinc-800 uppercase leading-none">Sultan Class</h4><p class="text-[8px] text-gray-400 font-bold uppercase mt-1">VIP Luxury</p></div></div><p class="text-sm font-black text-zinc-900 italic">Rp 25k</p></button>`;
