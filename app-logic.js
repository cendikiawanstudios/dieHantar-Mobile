...
const SULTAN_SERVICES = {
    trusted: [
        ...
        { id: 'dieMAKAN', icon: 'fa-utensils', color: 'text-red-600', bg: 'bg-red-100', label: 'dieMAKAN' },
        { id: 'dieKIRIM', icon: 'fa-box', color: 'text-yellow-600', bg: 'bg-yellow-100', label: 'dieKIRIM' },
        { id: 'dieBELANJA', icon: 'fa-shopping-bag', color: 'text-purple-600', bg: 'bg-purple-100', label: 'BELANJA' },
        { id: 'dieWASH', icon: 'fa-shirt', color: 'text-sky-600', bg: 'bg-white', label: 'dieWASH' }
    ],
    makan: [
        ...
        { id: 'dieMAKAN HEMAT', icon: 'fa-tags', color: 'text-orange-500', bg: 'bg-orange-50', label: 'MAKAN HEMAT' },
        { id: 'diePASAR', icon: 'fa-store', color: 'text-emerald-500', bg: 'bg-emerald-50', label: 'diePASAR' },
        { id: 'dieNGE DATE', icon: 'fa-heart', color: 'text-pink-500', bg: 'bg-pink-50', label: 'NGE DATE' }
    ],
    transport: [
...
const AppRenderer = {
    ...
    renderHome() {
        ...
        if (this.isDriver) {
            ...
            const gridHtml = SULTAN_SERVICES.trusted.slice(0, 7).map(s => `
    ...
    renderStatus() {
        const area = document.getElementById('status-area') || document.getElementById('global-status-container');
        if (!area) return;
    ...
    renderNav() {
        ...
        nav.innerHTML = items.map(i => `<button onclick="switchPage('${i.id}')" id="nav-${i.id}" class="flex flex-col items-center gap-1 text-gray-300 transition-all active:scale-95"><i class="fas ${i.icon} text-lg"></i><span class="text-[7px] font-black uppercase tracking-tighter">${i.label}</span></button>`).join('');
    },
...
async function syncAll() {
    try {
        ...
        if(document.getElementById('user-avatar')) document.getElementById('user-avatar').src = `https://ui-avatars.com/api/?name=${user.name}&background=fff&color=ea580c`;
        ...
        if (!isDriverRole && homeContainer) {
            if (activeOrder) {
                ...
                if (document.getElementById('map-slot') || homeContainer.innerHTML.trim() === "") {
                    progressValue = 0;
        ...
        if (profilePage && !profilePage.classList.contains('hidden')) {
            dieHantarUI.renderUserStats('user-stats-container', { trips: d.orders.length, points: user.points, years: 1 });
            ...
            if(autoTopupContainer) dieHantarUI.renderAutoTopupCard('auto-topup-slot', user.auto_topup);
        }

        dieHantarUI.renderHistory('history-list', d.orders);
        if(d.system.maintenance === 'ON') document.getElementById('maint-screen').classList.remove('hidden');
...
async function executeOrder(tipe, harga) {
    try {
        ...
        if (res.ok) { closeBooking(); SultanSound.playOrder(); alert("🔔 PESANAN SULTAN TERKIRIM!"); switchPage('home'); syncAll(); }
...
async function submitSultanReview() {
    ...
    await fetch('/api/review', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rating: currentRating, comment: document.getElementById('review-comment').value }) });
    if(document.getElementById('modal-review')) document.getElementById('modal-review').remove();
    alert("😇 Terimakasih Sultan! Poin Loyalitas Anda bertambah."); syncAll();
}
...
function setSultanRating(n) {
    ...
    for(let i=1; i<=5; i++) {
        ...
        star.className = (i <= n) ? 'fas fa-star text-yellow-400 scale-125' : 'far fa-star text-gray-200 scale-100';
    }
}
...
function startBooking(layanan, icon) {
    ...
    if(document.getElementById('modal-booking')) {
        ...
        document.getElementById('opt-icon-1').innerHTML = `<i class="fas ${icon} text-lg text-orange-600"></i>`;
        document.getElementById('modal-booking').classList.remove('hidden');
    }
}

const SultanSound = { playOrder() { const audio = new Audio('https://assets.mixkit.co/active_storage/sfx/2571/2571-preview.mp3'); audio.play().catch(() => {}); } };
function closeBooking() { document.getElementById('modal-booking').classList.add('hidden'); }
function logout() { localStorage.clear(); window.location.href = 'login.html'; }
async function executeWithdraw() { alert("✅ Tarik tunai sukses!"); syncAll(); }